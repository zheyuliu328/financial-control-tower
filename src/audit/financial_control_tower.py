...
class FinancialControlTower:
    ...
    def reconcile_operations_finance(self):
        ...
        if not missing_in_fin.empty:
            ...
            print(f"   风险级别: HIGH - 货物已发出但未记录收入")
            
            # 显示前5个案例
            print(f"\n   示例案例 (前5笔):")
            for idx, row in missing_in_fin.head(5).iterrows():
        ...
        if not amount_mismatch.empty:
            ...
            print(f"   风险级别: MEDIUM - 业务金额与财务金额不一致")
            
            # 显示前5个案例
            print(f"\n   示例案例 (前5笔):")
            for idx, row in amount_mismatch.head(5).iterrows():
    ...
    def audit_supply_chain_risks(self):
        ...
        if not timing_fraud.empty:
            ...
            print(f"   风险级别: CRITICAL - 发货日期早于订单日期")
            print(f"   业务含义: 先货后票 / 虚假订单补录 / 数据录入错误")
            
            # 显示案例
            print(f"\n   示例案例 (前3笔):")
            for idx, row in timing_fraud.head(3).iterrows():
        ...
        if not negative_margin.empty:
            ...
            print(f"   风险级别: MEDIUM - 利润为负的正常订单")
            print(f"   业务含义: 亏本销售 / 促销活动 / 价格录入错误")
            ...
            # 显示案例
            print(f"\n   示例案例 (前3笔最严重的):")
            for idx, row in negative_margin.nsmallest(3, 'profit').iterrows():
